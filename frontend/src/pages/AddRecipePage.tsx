  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      if (res.ok) {
        ...
        setError(errData.message || "Erreur de création");
      }
    } catch (err) {
    ...
    <div style={{ maxWidth: '600px', margin: '2rem auto', padding: '2rem', background: 'white', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        <div>
          ...
          <input {...register('name', { required: "Le nom est requis" })} style={{ width: '100%', padding: '0.8rem', borderRadius: '8px', border: '1px solid #ddd' }} placeholder="Ex: Boeuf Bourguignon" />
        ...
        <div>
          ...
          <input {...register('countryOfOrigin')} style={{ width: '100%', padding: '0.8rem', borderRadius: '8px', border: '1px solid #ddd' }} placeholder="Ex: France" />
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem', background: '#f5f6fa', padding: '1rem', borderRadius: '8px' }}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
            <input type="checkbox" {...register('specialEquipmentRequired')} /> Matériel spécifique (Robot, siphon...)
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
